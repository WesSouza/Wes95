import { renderWMF } from './wmf';

export type BobAnim = [number, number, number];

export type BobPose = [number, number, number, number, number];

export class BobPack {
  names: string[] = [];
  items: Record<string, ArrayBuffer> = {};
  ...
  constructor(buffer: ArrayBuffer) {
    ...
    const offsetTable = new Uint32Array(
      buffer.slice(10, 10 + (itemCount + 1) * 4),
    );
    const rawNameTable = new Uint8Array(
      buffer,
      10 + offsetTable.byteLength,
      nameTableSize,
    );
...
function readBlobArray(
  ...
  for (let i = 0; i < count; i++) {
    const blob = buffer.slice(
      baseStart + indices[i],
      baseStart + indices[i + 1],
    );
    result.push(blob);
  }
...
function readGroupedBlobArray(
  buffer: ArrayBuffer,
  baseStart: number,
  indexStart: number,
  indexEnd: number,
): Record<number, ArrayBuffer[]> {
  if (indexStart == indexEnd) {
    return {};
  }

  const result: Record<number, ArrayBuffer[]> = {};
  const dv = new DataView(buffer);
...
export class BobResource {
  width: number;
  height: number;
  rawPayloads: ArrayBuffer[] = [];
  riffs: Record<number, ArrayBuffer> = {};
  riffCache: Record<number, string> = {};
  rawAnims: Record<number, ArrayBuffer[]> = {};
  strings: Record<number, ArrayBuffer[]> = {};
  anims: Record<number, Record<number, BobAnim[]>> = {};
  poses: Record<number, BobPose[]> = {};
  ...
  constructor(buffer: ArrayBuffer) {
    ...
    this.rawPayloads = readBlobArray(
      buffer,
      payloadBase,
      payloadOffsetBase,
      riffBase,
    ...
    this.riffCache = {};
    this.rawAnims = readGroupedBlobArray(
      buffer,
      animBase,
      animOffsetBase,
      stringBase,
    );
    this.strings = readGroupedBlobArray(
      buffer,
      stringBase,
      stringOffsetBase,
      buffer.byteLength,
  ...
  readPoses() {
    ...
    for (let i = 0; i < this.rawPayloads.length; i++) {
      ...
      const pose: [number, number, number, number, number][] = [];
  ...
  clearRiffCache() {
    for (const key of Object.keys(this.riffCache)) {
      URL.revokeObjectURL(this.riffCache[Number(key)]);
    }
    this.riffCache = {};
  }
}
  ...
  if (wireframe) {
    ...
    for (let i = 0; i < pose.length; i++) {
      ...
      ctx.strokeRect(
        piece[1] * scaleX,
        piece[2] * scaleY,
        (piece[3] - piece[1]) * scaleX,
        (piece[4] - piece[2]) * scaleY,
      );
    }
  }
}
...
export class AniPlayer {
  ...
  callback: ((x: boolean) => void) | null = null;
  onPoseChange: (() => void) | null = null;
  pose: BobPose[] | null = null;
  currentAnim: Record<number, BobAnim> = {};
  commandNumber = 0;

  constructor(public resource: BobResource) {
  ...
  playAnimation(id: number, animation = 0) {
    this.stopAnimation();

    const animGroup = this.resource.anims[id];
    const anim = animGroup[animation];
  ...
  stepAnimation() {
    ...
    while (this.timer == null) {
      const command: BobAnim = this.currentAnim[this.commandNumber];
  ...
  draw(canvas: HTMLCanvasElement, wireframe = false) {
    if (!this.pose) {
      return;
    }
