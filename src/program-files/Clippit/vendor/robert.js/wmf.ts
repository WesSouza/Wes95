...
function readColorRef(dv: DataView, p: number) {
...
function readPenObject(dv: DataView, p: number) {
  ...
  const lineJoin: CanvasLineJoin = 'round';
  let isNull = false;
  let shrink = false;
...
function readBrushObject(dv: DataView, p: number) {
...
function readFontObject(dv: DataView, p: number) {
  ...
  const ansiFacename = dv.buffer.slice(
    dv.byteOffset + p + 18,
    dv.byteOffset + p + 18 + 32,
  );
  const paddedFacename =
    String.fromCharCode(...new Uint8Array(ansiFacename)) + '\0';
...
function readSpecialHeader(dv: DataView, p: number) {
  return {
    ...
    checksum: dv.getUint32(p + 20, true),
    _size: 22,
  };
}

function readMetaHeader(dv: DataView, p: number) {
  return {
...
function addGDIObject<T>(objects: T[], obj: T) {
...
export function renderWMF(
  ...
  const objects: WMFObject[] = [];
  let fillMode: CanvasFillRule = 'evenodd';
  let offsetX = specialHeader.left,
    offsetY = specialHeader.top;
  const statedW = specialHeader.right - specialHeader.left;
  const statedH = specialHeader.bottom - specialHeader.top;
  const wantedW = right - left;
  const wantedH = bottom - top;
  let scaleX = wantedW / statedW,
    scaleY = wantedH / statedH;
  ...
  let windowOrgX = 0,
    windowOrgY = 0;
  let windowExtX = ctx.canvas.width,
    windowExtY = ctx.canvas.height;
  let inLineToSequence = false;

  const dcStack: DC[] = [];

  const w = specialHeader.right - specialHeader.left;
  ...
  while (position < buffer.byteLength) {
    ...
    switch (recordFunction) {
      ...
      case WMF_META.RESTOREDC:
        ...
        if (dcIndex < 0) {
          dcIndex = dcStack.length + dcIndex;
        } // convert to absolute
      ...
      case WMF_META.SETWINDOWORG:
        ...
        if (dbg) {
          console.log(`windowOrg: ${windowOrgX},${windowOrgY}`);
        }
        break;
      case WMF_META.SETWINDOWEXT:
        ...
        if (dbg) {
          console.log(`windowExt: ${windowExtX},${windowExtY}`);
        }
        offsetX = windowOrgX;
        offsetY = windowOrgY;
        scaleX = wantedW / windowExtX;
      ...
      case WMF_META.SELECTOBJECT:
        ...
        if (dbg) {
          console.log(`select ${selObjectID}`);
        }
        switch (selectedObject.type) {
          case 'pen':
            ctx.lineWidth = selectedObject.width ?? 0;
            ctx.strokeStyle = selectedObject.color ?? '';
            ctx.lineCap = selectedObject.lineCap ?? 'butt';
            ctx.lineJoin = selectedObject.lineJoin ?? 'round';
            ctx.setLineDash(selectedObject.lineDash ?? []);
            havePen = !selectedObject.isNull;
            shrinkMode = selectedObject.shrink ?? false;
            halfPenWidth = (selectedObject.width ?? 0) / 2;
            break;
          case 'brush':
            ...
            transparentBrush =
              brushColor == 'rgb(245,250,245)' || brushColor == 'rgb(4,4,4)';
            break;
          case 'font':
            if (dbg) {
      ...
      case WMF_META.DELETEOBJECT:
        ...
        delete objects[delObjectID];
        if (dbg) {
          console.log(`delete ${delObjectID}`);
        }
        break;
      case WMF_META.SETPOLYFILLMODE:
        fillMode =
          dv.getUint16(position + 6, true) == 2 ? 'nonzero' : 'evenodd';
        break;
      case WMF_META.MOVETO:
        const moveToY =
          (dv.getUint16(position + 6, true) - offsetY) * scaleY + bottom;
        const moveToX =
          (dv.getUint16(position + 8, true) - offsetX) * scaleX + left;
        ctx.beginPath();
      ...
      case WMF_META.POLYPOLYGON:
        ...
        const pointsPerPolygon = new Uint16Array(
          buffer,
          position + 8,
          polygonCount,
        );
        ...
        for (let i = 0; i < polygonCount; i++) {
          ...
          for (let j = 1; j < pointCount; j++) {
            ctx.lineTo(
              (points[j * 2] - offsetX) * scaleX + left,
              (points[j * 2 + 1] - offsetY) * scaleY + top,
            );
          }
          if (haveBrush) {
            ctx.fillStyle = brushColor;
      ...
      case WMF_META.ELLIPSE:
        const eBottom =
          (dv.getInt16(position + 6, true) - offsetY) * scaleY + top;
        const eRight =
          (dv.getInt16(position + 8, true) - offsetX) * scaleX + left;
        const eTop =
          (dv.getInt16(position + 10, true) - offsetY) * scaleY + top;
        const eLeft =
          (dv.getInt16(position + 12, true) - offsetX) * scaleX + left;
        ...
        if (dbg) {
          console.log(
            `eCentre:${eCentreX},${eCentreY} eRadius:${eRadiusX},${eRadiusY}`,
          );
        }
      ...
      case WMF_META.EOF:
        return;
      default:
        console.error(`unhandled function: ${recordFunction.toString(16)}`);
        break;
    }
